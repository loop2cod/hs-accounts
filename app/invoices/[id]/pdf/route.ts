...
function generatePrintHtml(
  ...
  if (totalItems === 0) {
    pages.push({ items: [], padTo: 20, hasFooter: true, pageIndex: 0, startIndex: 0 });
  }
  ...
  for (let p = 0; p < pages.length; p++) {
    ...
    let itemsHtml = page.items.map((item: any, i: number) => `
      <tr>
        <td style="border-left:1px solid #000;border-right:1px solid #000;padding:4px;text-align:center">${page.startIndex + i + 1}</td>
    ...
    const pageBreakStyle = p < pages.length - 1 ? ' style="page-break-after: always;"' : '';
    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:10px">
      ...
      <div style="flex:1;text-align:right">
        ...
        <div style="font-size:11px">
          ...
          ${pages.length > 1 ? `<div><strong>Page:</strong> ${p + 1} of ${pages.length}</div>` : ""}
        </div>
      </div>
    </div>
